...
部署 Halo 博客，并链接到不同 docker compose 部署的 `postgres SQL`​ 数据库。

## 一、在已有的 `postgres SQL`​ 创建新数据库 名称为 halo

```sh
docker exec -it lobe-postgres bash      # 进入容器
...
services:
  postgresql:
    ...
    environment:
      ...
      - 'POSTGRES_PASSWORD=uWNZugjBqixf8dxC'    # 密码（管理员 postgres 的密码）
    healthcheck:
...
services:
  halo:
    ...
    command:
      ...
      - --spring.r2dbc.password=uWNZugjBqixf8dxC
